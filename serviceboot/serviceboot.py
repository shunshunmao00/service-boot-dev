...
# 继承 WebsocketHandler, 用于支持处理Websocket请求
# 又因为 WebsocketHandler继承 web.RequestHandler, 所以自然能够处理HTTP Web请求
# 针对websocket请求主要要实现的接口是: on_message, on_ws_connection_close
class GatewayApi(tornado.websocket.WebSocketHandler):
    ...
    def on_ws_connection_close(self, close_code: int = None, close_reason: str = None):
    ...
    async def get(self, *args, **kwargs):
        ...
        if self.request.headers.get("Upgrade", "").lower() == "websocket":
            ...
            if result['status'] == 'ok':
                ...
                # 判断响应是否是chunk传输
                transfer_encoding_header = response.headers.get("Transfer-Encoding")
